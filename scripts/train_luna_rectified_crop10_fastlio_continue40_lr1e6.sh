...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0}" python -u stereo_lidar.py \
  --checkpoint_dir outputs/luna_rectified_crop10_fastlio_continue40_lr1e6_exclude2 \
  --train_datasets luna \
  --test_datasets luna \
  ...
  --resume_ckpt outputs/luna_rectified_crop10_fastlio_from_pretrained_ep5_exclude2/ckpt_best.pth \
  --strict_resume 0 \
  --max_disp 192 \
  --image_size 256 512 \
