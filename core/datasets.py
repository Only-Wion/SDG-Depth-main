...
class KITTI_completion(StereoDataset):
    def __init__(self, aug_params=None, root='datasets/kitti_mono', image_set='training',
        ...
        for samp in disp_list:
            sample_path = Path(samp)
            sample_name = sample_path.parents[3].name
            img_num = sample_path.stem
...
def fetch_dataloader(args):
    ...
    for dataset_name in args.train_datasets:
        ...
        elif 'luna' in dataset_name:
            new_dataset = LunaOrganized(aug_params, root=args.luna_root, image_set='training', args=args)
            logging.info(f"Adding {len(new_dataset)} samples from LunaOrganized")
        else:
